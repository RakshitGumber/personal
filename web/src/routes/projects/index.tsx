  ...
  const filteredProjects = useMemo(
    () =>
      projectPosts.filter((project) => {
        const matchesQuery =
          !query ||
          `${project.title} ${project.summary} ${project.stack.join(" ")}`.toLowerCase().includes(query.toLowerCase());
    ...
    <main className="space-y-8">
      ...
      <header className="space-y-3 border-b border-line pb-6">
        ...
        <div className="space-y-3">
          <SearchBar label="Search projects" value={query} onChange={setQuery} placeholder="Search projects..." className="max-w-md" />
          <div className="flex flex-wrap gap-2">
            {[["language", languages, language, setLanguage], ["type", types, type, setType], ["status", statuses, status, setStatus]].map(
              ([label, values, active, setter]) => (
                <div key={String(label)} className="flex flex-wrap items-center gap-2">
                  <p className="text-xs uppercase tracking-[0.12em] text-muted">{label}</p>
                  {(values as string[]).map((value) => (
                    <button key={value} type="button" onClick={() => (setter as (value: string) => void)(value)}>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        {filteredProjects.length === 0 ? <article className="card p-5 text-muted">No projects matched your filters.</article> : null}
      </section>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4">
          <div className="card w-full max-w-xl p-6">
            ...
            <div className="mt-4 flex gap-4 text-sm">
              <a href={previewProject.demoLink} className="text-accent" rel="noopener noreferrer" target="_blank">
                demo
              </a>
              <a href={previewProject.sourceCodeLink} className="text-accent" rel="noopener noreferrer" target="_blank">
                source
              </a>
            </div>
          </div>
